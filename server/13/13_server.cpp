...
static void entry_set_ttl(Entry *ent, int64_t ttl_ms) {
    if (ttl_ms < 0 && ent->heap_idx != (size_t)-1) {
        // erase an item from the heap 这里应该是删除的意思
...
static void do_expire(std::vector<std::string> &cmd, std::string &out) {
    ...
    key.key.swap(cmd[1]);
    key.node.hcode = str_hash((uint8_t *)key.key.data(), key.key.size());
    // 其实有个疑惑 直接传cmd的内容不久行了么 为什么还有搞个？？？
    // 好处其实好像还行侵入式数据结构 也就是下面hm_lookup 还需要键的比较
    // 但是我们没传进去 我们是用一个node的地址 但是通过这个node的地址
    // 也是可以获得  key.key.swap(cmd[1]); 这里放的key的
    HNode *node = hm_lookup(&g_data.db, &key.node, &entry_eq);
    if (node) {
        Entry *ent = container_of(node, Entry, node);
...
static void entry_del(Entry *ent) {
    switch (ent->type) {
    case T_ZSET: // 这其实体现了嵌入式数据结构的好吃 可以选择是否启用有续集
